...
docker run -d  --name=amplicon-${AMPLICON_ENV} -p ${AMPLICON_ENV_PORT}:8000 -v /home/ubuntu/.aws:/root/.aws  -v /home/ubuntu/AmpliconRepository-dev/logs:/srv/logs -v /home/ubuntu/AmpliconRepository-dev/caper:/srv/caper/ -w /srv/caper  --env GOOGLE_SECRET_KEY --env GLOBUS_SECRET_KEY --env DB_URI --env DB_NAME  --env S3_STATIC_FILES --env S3_FILE_DOWNLOADS -t genepattern/amplicon-repo:${AMPLICON_ENV} /srv/run-manage-py.sh
