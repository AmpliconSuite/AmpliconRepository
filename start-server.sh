...
# USE_FEATURE_INDEX_SEARCH switches the site search onto the feature index.
# It is read by settings.py at import, which means the *container* has to carry
# it: setting it in config.sh alone does nothing, because `docker run` fixes the
# environment at creation and a `docker restart` re-runs the same environment.
# Passing it here unconditionally is safe -- `--env NAME` with the variable
# unset passes nothing, and settings.py then defaults it to False.
#
# So turning the search over is: set it in config.sh on that host, then
# stop + rm + re-run this script. A restart will not do it, and nothing in the
# UI will say the flag failed to take -- the old search path just keeps
# answering.

docker run -d --network="host" --restart ${AMPLICON_RESTART_POLICY} --memory ${AMPLICON_MEMORY_LIMIT}  --name=amplicon-${AMPLICON_ENV} -p ${AMPLICON_ENV_PORT}:8000 -v /home/ubuntu/.aws:/root/.aws  -v ${CAPER_ROOT}:${CAPER_ROOT}  -v ${CAPER_ROOT}/logs:/srv/logs -v ${CAPER_ROOT}:/srv/ -w /srv/caper  -v ${CAPER_ROOT}/.git:/srv/.git --env CAPER_ROOT --env NEO4J_PASSWORD_SECRET --env EMAIL_HOST_USER --env EMAIL_HOST_PASSWORD --env DJANGO_SECRET_KEY --env GOOGLE_SECRET_KEY --env GLOBUS_SECRET_KEY --env DB_URI_SECRET --env DB_NAME --env SITE_URL --env S3_STATIC_FILES --env S3_FILE_DOWNLOADS --env SECRET_KEY --env USE_FEATURE_INDEX_SEARCH  -t genepattern/amplicon-repo:${AMPLICON_ENV} /srv/run-manage-py.sh
