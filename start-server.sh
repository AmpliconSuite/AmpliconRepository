...
docker run -d  --name=amplicon-${AMPLICON_ENV} -p ${AMPLICON_ENV_PORT}:8000 -v /home/ubuntu/.aws:/root/.aws  -v ${CAPER_ROOT}/logs:/srv/logs -v ${CAPER_ROOT}:/srv/ -w /srv/caper  -v ${CAPER_ROOT}/.git:/srv/.git  -t genepattern/amplicon-repo:${AMPLICON_ENV} /srv/run-manage-py.sh
