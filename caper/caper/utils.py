...
# The collections behind fs_handle.  Held once, like every other handle above:
# db_handle['fs.chunks'] builds a new Collection object on each subscript, so a
# caller that reaches for it repeatedly gets a different object every time.
...
# Visibility lives in its own module so the standalone scripts, which have no
# Django, can import it too.  Re-exported here because the call sites that
# predate the split all say `from .utils import ...`.
