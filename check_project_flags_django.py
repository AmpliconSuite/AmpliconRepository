#!/usr/bin/env python
"""
Django management command to check project flags.

Usage:
    cd /path/to/caper
    python manage.py shell
    >>> exec(open('check_project_flags_django.py').read())
"""
...
if not all_projects:
    ...
    print(f"  - NOT SET:  {missing_current:4d} **")
    print(f"\n'delete' flag:")
    print(f"  - True:     {delete_true:4d}")
    print(f"  - False:    {delete_false:4d}")
    print(f"  - NOT SET:  {missing_delete:4d} **")
    
    # Warnings
    if missing_current > 0:
        print(f"\n** WARNING: {missing_current} project(s) missing 'current' flag!")
        print("   These projects may not appear in filtered queries.")
    
    if missing_delete > 0:
        print(f"\n** WARNING: {missing_delete} project(s) missing 'delete' flag!")
    ...
    if missing_current > 0 or missing_delete > 0:
        ...
        for project in all_projects:
            ...
            if current_status == 'missing' or delete_status == 'missing':
                ...
                print(f"  current: {current_value} {'** MISSING' if current_status == 'missing' else ''}")
                print(f"  delete:  {delete_value} {'** MISSING' if delete_status == 'missing' else ''}")
                print()
    ...
    if not old_versions:
        ...
        if orphaned_count == 0:
            ...
            print(f"** FOUND {orphaned_count} ORPHANED OLD VERSION(S) **")
