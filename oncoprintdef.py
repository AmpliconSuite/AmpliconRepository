...
for key in table:
  ...
  if 'ecDNA' in table[key]:
    curr_row.append('TRUNC')
  else:
    curr_row.append(None)
  if 'BFB' in table[key]:
    curr_row.append('INFRAME')
  else:
    curr_row.append(None)
  if 'Complex non-cyclic' in table[key]:
    curr_row.append('MISSENSE')
  else:
    curr_row.append(None)
...
for ind in df.index:
  for i in range(4):
    ...
    if cells_per_sample[ind][i] != None:
      if cells_per_sample[ind][i] == 'TRUNC':
        x['gene'] = 'ecDNA'
      if cells_per_sample[ind][i] == 'INFRAME':
        x['gene'] = 'BFB'
      if cells_per_sample[ind][i] == 'MISSENSE':
        x['gene'] = 'Complex non-cyclic'
      if cells_per_sample[ind][i] == 'AMP':
